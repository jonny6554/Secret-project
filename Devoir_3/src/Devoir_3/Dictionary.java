...
/**
 * Une instance de Dictionary contient des pairs d'éléments clef-valeur
 * (key-value) qui peuvent être manipulé avec les méthodes de la classe. Un map
 * peut contenir des associations clef-valeur dupliqués. Lorsque c'est le cas,
 * au besoin, la méthode appelée fait référence à la dernière valeur pour une
 * clef donnée.
 * 
 * Un dictionnaire est analogue à une matrice de variables: chaque variable a un
 * nom (la clef) et chaque variable est associé à une valeur (valeur de type
...
public class Dictionary implements Map<String, Token> {
	...
	public void replace(String key, Token value) throws NoSuchElementException,
			NullPointerException {
		// Déclaration des variables et dictionnaire des données
		boolean found = false; // Seulement vrai si on trouve la clef à
								// remplacer.
	...
	private void lengthenTheDictionary() {
		...
		// On crée les éléments null qui ne sont pas présent dans le vieux
		// dictionnaire
	...
	/**
	 * Cette méthode réorganise le stack lorsqu'il y a exactement <b> une </b>
	 * valeur null centré entre deux valeurs non null ou une valeur null au
	 * début de la matrice <code> dictionary</code>.
	 */
